...
public class ABUpdateMgr : MonoBehaviour
{
    ...
    //资源服务器IP
    private string serverIP = "ftp://10.12.14.203";                                                       
    ...
    public void CheckUpdate(UnityAction<bool> overCallBack, UnityAction<string> updateInfoCallBack, UnityAction<float> updateProCallBack,UnityAction allOverCallBack)
    {
                    ...
                    if (isOver)
                    {
                        ...
                        DownLoadABFile((isOver) =>
                        {
                            if (isOver)
                            {
                                ...
                                updateProCallBack(1f);
                                allOverCallBack?.Invoke();
                            }
                            overCallBack(isOver);
                        }, updateInfoCallBack);
                    }
    ...
    private bool DownLoadFile(string fileName, string localPath)
    {
        try
        {
            ...
            NetworkCredential n = new NetworkCredential("1", "88888888");
            req.Credentials = n;
            //3.其它设置
            //  设置代理为null
            req.Proxy = null;
